...
from mlpproc.blocks import Blck_If
from mlpproc.errors import WarningMode


class TestCommands:
    file_name = "test_commands"

    def single_runtest(self, test_in: str, name: str, test_out: str) -> None:
        pre = Preprocessor()
        pre.warning_mode = WarningMode.PRINT
        assert pre.process(test_in, name) == test_out

    def runtests(self, test: List[Tuple[str, str]], name: str) -> None:
    ...
    def test_def(self) -> None:
        test = [
            ...
            (
                "{% def add(pha,alpha,lpha) (pha,alpha)lpha %}hello{% add 1 2 3 %}",
                "hello(1,2)3",
            ),
            (
                "{% def f(a,b) a+b %}{% def f(a) {% f a 0 %} %}{% f 1 2 %}; {% f 1 %}",
                "1+2; 1+0",
            ),
        ]
    ...
    def test_begin_end(self) -> None:
        test = [
            ...
            (
                "{% def foo bar %}{% def hello {% begin %}foo{% end %} %}{% hello %}",
                "bar",
            ),
            (
                "{% def foo bar %}{% def foo2 {% call foo %} %}{% foo2 %}{% def foo yoyo %}"
                "{% foo2 %}",
                "baryoyo",
            ),
        ]
    ...
    def test_include(self) -> None:
        ...
        test = [
            (
                "hello",
                "bonjour:{% include test.out %}:guten tag",
                "bonjour:hello:guten tag",
            ),
            ("{% def a b %}", "bonjour:{% include test.out %}:{% a %}", "bonjour::b"),
            (
                "{% def a b %}{% c %}",
                "bonjour{% def c d %}:{% include test.out %}:{% a %}",
                "bonjour:d:b",
            ),
            (
                "{% def a b %}{% c %}",
                "bonjour{% def c d %}:{% include -v test.out %}:",
                "bonjour:{% def a b %}{% c %}:",
            ),
            (
                "<def a b><c>",
                "bonjour{% def c d %}:{% include -b < -e > test.out %}:{% a %}",
                "bonjour:d:b",
            ),
        ]
    ...
    def test_replace(self) -> None:
        test = [
            ...
            (
                "foo{% block -a %}{% replace foo bar %}foo yfoo{% endblock %}foo",
                "foobar ybarfoo",
            ),
            ("afoo{% replace foo bar %}{% replace aba yo %}fooabr", "yorbarabr"),
            ("afoo{% replace --ignore-case foo bar %}FoOfOo FOO", "abarbarbar bar"),
            (
                "{% replace -w foo bar %}foo(afoo1foo+foo foo foo",
                "bar(afoo1foo+bar bar bar",
            ),
    ...
    def test_cut_paste(self) -> None:
        test = [
            (
                "{% cut %}hello there!{% endcut %}hello:{% paste %}",
                "hello:hello there!",
            ),
            (
                "{% cut a %}content a{% endcut %}{% cut b %}content b{% endcut %}{% paste a %}"
                "{% paste b %}",
                "content acontent b",
            ),
            (
                "{% def foo hi %}{% cut %}{% def foo bar1 %}{% endcut %}{% foo %}"
                "{% paste %}{% foo %}",
                "hibar1",
            ),
            (
                "{% def foo hi %}{% cut --pre-render %}{% def foo bar1 %}{% endcut %}"
                "{% foo %}{% paste %}{% foo %}",
                "bar1bar1",
            ),
            (
    ...
    def test_block(self) -> None:
        test = [
            (
                "hello{% def a c %}{% comment %} how are {% def a b %}"
                " you {% endcomment %} {% a %}",
                "hello c",
            ),
            (
                "text{% void %}{% def name john %}hello this is a comment"
                "{% endvoid %}\n{% name %}",
                "text\njohn",
            ),
            ("{% verbatim %}{% hello %}{% endverbatim %}", "{% hello %}"),
            (
                "{% verbatim %}some text with {% verbatim %}nested verbatim"
                "{% endverbatim %}{% endverbatim %}",
                "some text with {% verbatim %}nested verbatim{% endverbatim %}",
            ),
            ("{% repeat 5 %}yo{% endrepeat %}", "yoyoyoyoyo"),
            (
                "{% label foo %}lala{% atlabel foo %}bar{% endatlabel %}"
                "yoyo{% label foo %}oups",
                "barlalayoyobaroups",
            ),
            ("{% atlabel yo %}bonjour{% endatlabel %}{% label yo %}", "bonjour"),
            (
                "{% atlabel yo %}bjr{% endatlabel %}{% label yo %}..{% label yo %}",
                "bjr..bjr",
            ),
            (
                "{% atlabel yo %}bjrst{% endatlabel %}{% block %}hi{% label yo %}yy"
                "{% endblock %}",
                "hibjrstyy",
            ),
            (
                "{% atlabel yo %}bonjour{% endatlabel %}{% label yo %}***\n\n{% block %}"
                "nested:{% label yo %}{% endblock %}\n{% repeat 2 %}{% label yo %}"
                "{% endrepeat %}***{% label yo %}",
                "bonjour***\n\nnested:bonjour\nbonjour***bonjour",
            ),
            (
                "{% def f bar %}{% def g hi %}{% block -d %}{% def f foo %}{% f %}{% undef g %}"
                "{% endblock %}{% f %}{% g %}",
                "foobarhi",
            ),
            (
                "{% label a %}({% block -a %}{% label a %}{% atlabel a %}yo{% endatlabel %}"
                "{% endblock %})",
                "(yo)",
            ),
            (
                "{% label a %}({% block -a %}{% label a %}{% atlabel a %}yo{% endatlabel %}"
                "{% endblock %}){% atlabel a %}boo{% endatlabel %}",
                "boo(booyo)",
            ),
            (
                "{% label a %}({% block -al %}{% label a %}{% atlabel a %}yo{% endatlabel %}"
                "{% endblock %}){% atlabel a %}boo{% endatlabel %}",
                "boo(yo)",
            ),
        ]
    ...
    def test_if_find_elif(self) -> None:
        test_match = [
            ("qmldkf", (-1, -1, None)),
        ...
        for string, result in test_match:
            print("====== TEST ======")
            pre = Preprocessor()
            assert Blck_If().find_elifs_and_else(pre, string) == result
    ...
    def test_if(self) -> None:
        test = [
            ...
            (
                "{% def foo bar %}{% if {% foo %}==bar %}yes{% def foo nn %}{% endif %}{% foo %}",
                "yesnn",
            ),
            (
                "{% def foo bar %}{% if {% foo %}!=bar %}yes{% def foo nn %}{% endif %}{% foo %}",
            ...
            (
                "{% if ndef if %}hello{% elif ndef def %}there{% elif def if %}general{% else %}"
                "kenobi{% endif %}",
                "general",
            ),
            (
            ...
            (
                """{% def foo bar %}{% if def foo %}{% if {% foo %}!=bar %}{% def foo si %}"
                "{% else %}{% def foo la %}{% endif %}{% else %}no foo{% endif %}{% foo %}""",
                "la",
            ),
        ]
