...
from preproc.errors import PreprocessorError, PreprocessorWarning
...
def runtest_error(filename, in_text, error_name, error_line, error_char):
	...
	try:
		...
		assert err.name == error_name and err.line == error_line and err.char == error_char
		return
	assert False and "No error caught"

def runtest_warning(filename, in_text, error_name, error_line, error_char):
	pre = Preprocessor()
	try:
		pre.process(in_text, filename)
